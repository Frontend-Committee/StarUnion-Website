...
/* ── team member card ── */
const TeamCard = ({ member, accentColor }) => (
  <motion.div
    whileHover={{ y: -4 }}
    className="min-w-[220px] max-w-[260px] flex-shrink-0 rounded-[16px] overflow-hidden border border-primary/30 bg-[#160d2e]"
  >
    {/* Banner */}
    <div
    ...
    >
      <div className="absolute inset-0 flex items-center justify-center">
        <p className="text-[9px] text-white/60 font-bold text-center leading-tight px-3">
          ...
          <span className="text-white/40 mt-1 block">Learn | Build | Grow</span>
        </p>
      </div>
    </div>
    ...
    <div className="p-4 pt-3">
      ...
      <p className="text-white text-[13px] mt-1 font-semibold">{member.role}</p>
...
const ProjectCard = ({ project, accentColor }) => (
  ...
  >
    ...
    <div className="p-4">
      ...
      <button className="px-4 py-[6px] text-[12px] text-secondary border border-secondary rounded-md hover:bg-secondary/10 transition-colors font-bold">
        View Details
      </button>
    </div>
  </motion.div>
...
export default function CommitteeDetailsPage() {
  ...
  const accentColor = committee
    ? ACCENT_COLORS[committee.accent]
    : ACCENT_COLORS.teal;
    ...
    <section className="min-h-screen bg-gradientBg3 py-8 px-4 md:px-8">
      <div className="max-w-[1200px] mx-auto">
        ...
        <motion.button
          ...
          className="text-white hover:text-secondary transition-colors mb-8 flex items-center gap-2 group"
        ...
        <motion.div
          ...
          className="flex gap-5 overflow-x-auto pb-4 mb-14 scrollbar-hide"
        >
