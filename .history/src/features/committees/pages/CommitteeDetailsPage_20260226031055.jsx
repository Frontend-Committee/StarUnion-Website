...
const ProjectCard = ({ project, delay = 0 }) => (
  <ScrollAnimation variant="fade-up" delay={delay} className="flex-shrink-0">
    <motion.div
      ...
      transition={{ type: "spring", stiffness: 300, damping: 22 }}
      className="w-[185px] rounded-[14px] overflow-hidden border border-primary/20 bg-[#0d0820] flex flex-col"
    ...
    >
      ...
      <div className="relative w-full overflow-hidden" style={{ height: 200 }}>
        ...
        <div
          ...
          style={{
            background:
              "linear-gradient(180deg, #0d1117 0%, #141920 45%, #1a1230 100%)",
          }}
        />

        {/* Fake UI card rows — screenshot effect */}
        <div className="absolute inset-0 p-2.5 pt-3 flex flex-col gap-[5px]">
          {[85, 70, 60, 75].map((w, i) => (
            <div
              key={i}
              className="rounded-[6px] p-2 flex flex-col gap-[4px]"
              style={{
        ...
        <div
          ...
          style={{
            background:
              "linear-gradient(to top, rgba(8,2,22,1) 0%, rgba(8,2,22,0.75) 45%, rgba(8,2,22,0.1) 100%)",
          }}
        />

        {/* Overlay text — "Decentralized. Scalable." + "APP Design" */}
        <div className="absolute bottom-0 left-0 right-0 px-3 pb-3">
          <p className="text-white/55 text-[8px] font-semibold tracking-[0.08em] uppercase mb-[1px]">
            {project.subtitle}
          </p>
          <h4 className="text-white font-black text-[19px] leading-none tracking-tight">
            {project.type}
          </h4>
        </div>
      ...
      {/* ── Bottom bar ── */}
      <div className="px-3 py-[10px] flex items-center justify-between gap-2 bg-[#0d0820]">
        <p className="text-tertiary font-bold text-[12px] leading-tight flex-1 truncate">
          {project.title}
        </p>
        <button className="flex-shrink-0 px-3 py-[5px] text-[10px] text-white bg-primary border border-primary rounded-[6px] hover:bg-primary/80 transition-colors font-bold whitespace-nowrap">
          View Details
        </button>
      </div>
    </motion.div>
