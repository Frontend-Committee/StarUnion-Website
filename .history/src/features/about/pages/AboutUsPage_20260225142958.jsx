      ...
      {open && (
        <div className="absolute top-[calc(100%+8px)] left-0 min-w-[220px] bg-[rgba(36,19,82,0.97)] border border-white/18 rounded-xl shadow-lg overflow-hidden z-50">
          {items.map((item) => (
            <button
              key={item}
              className="block w-full text-left px-4 py-2.5 text-xs text-white hover:text-w bg-transparent border-none cursor-pointer hover:bg-white/9 transition-colors duration-150"
            >
              {item}
            </button>
          ))}
