...
function StarLogo() {
  return (
    <svg width="110" height="110" viewBox="0 0 120 120" fill="none" xmlns="http:
  ...
  return (
    <div className="relative inline-block">
      <motion.button
        onClick={() => setOpen((o) => !o)}
        whileTap={{ scale: 0.97 }}
        className="flex items-center gap-2 px-5 py-2.5 border w-80 justify-center border-white rounded-md bg-white text-[#452798]  text-sm font-bold cursor-pointer hover:bg-white/90 transition-colors duration-200"
      >
        {label}
...
<div className="absolute inset-0 bg-[#BDADE9]/45 " />      <div className="absolute inset-0 bg-black/30 group-hover:bg-black/45 transition-colors duration-300" />
      <div className="absolute inset-0 flex flex-col items-center justify-between py-5 px-4">
        ...
        <motion.button
          ...
          whileTap={{ scale: 0.95 }}
          className="px-6 py-1.5 text-xs font-semibold text-[#452798] bg-white border border-primary  rounded-md hover:bg-white/90 transition-colors duration-200 shadow-md"
        >
          View Details
        </motion.button>
      </div>
