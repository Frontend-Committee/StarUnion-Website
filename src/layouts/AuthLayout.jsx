...
export default function AuthLayout({ children }) {
  ...
  return (
    <div className="relative flex flex-col min-h-screen">
      ...
      <main className={`flex-1 relative ${isNavigating ? "opacity-50 pointer-events-none" : "opacity-100"}`}>
        ...
        <div className="absolute inset-0 bg-[#311f62]/60 overflow-y-auto overflow-x-hidden">
          <div className="flex flex-col items-center justify-start min-h-full p-4 py-20 md:justify-center md:py-10">
            <Link to="/" className="fixed z-50 cursor-pointer md:absolute top-6 left-6 md:top-8 md:left-8">
              <img src={Logo} alt="Logo" className="transition-transform rounded-full shadow-lg size-11 md:size-12 hover:scale-110 active:scale-95 bg-white/10 backdrop-blur-sm" />
            </Link>
